...
def test_ixia_demo_32_bgp_fib(ixiahost,testbed, duthost):
    ...
    if  flowStat['Tx Frames'] ==  flowStat['Rx Frames']:
        result = True
        logger_msg(u'与DUT建立BGP邻居状态正确且协议启动时间正常，路由验证流量无丢包。测试成功结束。')
        logger_msg(u'Fib 容量为16000')
        logger_msg(u'Fib 容量测试结束')
    else:
        result = False
